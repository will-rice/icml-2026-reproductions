...
6. Before publishing, commit the exact attested source to the shared repository on `main` under `submissions/<slug>/` with its `.gitignore` negation block (`!submissions/<slug>/` and `!submissions/<slug>/**`); `/tmp` staging worktrees are ephemeral and their loss forces reimplementation. Use `publish-deployment` for the dedicated Space. Require the allowlisted owner, exact paper and challenge tags, exact attested SHA, and `RUNNING` runtime. Verify the live Space serves every project file, especially every `pages/*.md` — judges read only the served logbook. Then take a fresh assessed snapshot and use `attest-submission`. Space existence does not prove submission.
