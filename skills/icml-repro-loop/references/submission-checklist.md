...
- [ ] Create every validation worktree under `.worktrees/` and every
  staging clone or scratch checkout under `scratch/` (a workspace symlink
  onto the large `/data` drive), never in `/tmp`: systemd wipes `/tmp` on
  boot, `candidate-census` polices only workspace worktrees, and `/tmp`
  staging is how sixty gigabytes of validated sources nearly vanished.
  Remove your worktree and scratch dirs when the iteration releases.
