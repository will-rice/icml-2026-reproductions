#!/usr/bin/env python3
"""
Objaverse retrieval server entry point.

This module provides the main entry point for running the Objaverse retrieval server.
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Objaverse retrieval server for semantic object search using CLIP",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Preload the Objaverse retriever (includes CLIP model loading) on server "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to ObjectThor assets directory. If not specified, uses environment "
        "variable OBJAVERSE_DATA_PATH or default 'data/objathor-assets'.",
    )
    parser.add_argument(
        "--objaverse-preprocessed-path",
        type=str,
        default=None,
        help="Path to preprocessed data directory. If not specified, uses environment "
        "variable OBJAVERSE_PREPROCESSED_PATH or default "
...
def main() -> int:
    """Main entry point for the Objaverse retrieval server.
    ...
    try:
        ...
        server = ObjaverseRetrievalServer(
            ...
            objaverse_data_path=args.objaverse_data_path,
            objaverse_preprocessed_path=args.objaverse_preprocessed_path,
            objaverse_top_k=args.objaverse_top_k,
        )
        server.start()

        console_logger.info(
            f"Objaverse retrieval server running on {args.host}:{args.port}"
        )
