...
4. **Constrained Preference Optimization (CPO)**: Audited 180 grid configurations. Certified that the exact constrained RLHF objective for $\gamma > 0$ is unbounded as $\Delta \to +\infty$. Substituting the reference policy margin yields a stationary DPO-like loss with adaptive shift $\Delta_{ref} + (r + \Omega_{ref})/\beta$. Local outcome: `consistent`.
