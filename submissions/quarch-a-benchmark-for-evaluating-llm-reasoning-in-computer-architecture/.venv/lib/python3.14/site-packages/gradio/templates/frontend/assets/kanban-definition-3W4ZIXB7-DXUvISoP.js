...
`+e);let o=a.db.getData(),u=s();u.htmlLabels=!1;let m=l(n),h=m.append(`g`);h.attr(`class`,`sections`);let g=m.append(`g`);g.attr(`class`,`items`);let _=o.nodes.filter(e=>e.isGroup),v=0,y=[],b=25;for(let e of _){let t=u?.kanban?.sectionWidth||200;v+=1,e.x=t*v+(v-1)*10/2,e.width=t,e.y=0,e.height=t*3,e.rx=5,e.ry=5,e.cssClasses=e.cssClasses+` section-`+v;let n=await p(h,e);b=Math.max(b,n?.labelBBox?.height),y.push(n)}let x=0;for(let e of _){let t=y[x];x+=1;let n=u?.kanban?.sectionWidth||200,r=-n*3/2+b,i=r,a=o.nodes.filter(t=>t.parentId===e.id);for(let t of a){if(t.isGroup)throw Error(`Groups within groups are not allowed in Kanban diagrams`);t.x=e.x,t.width=n-1.5*10;let r=(await d(g,t,{config:u})).node().getBBox();t.y=i+r.height/2,await f(t),i=t.y+r.height/2+10/2}let s=t.cluster.select(`rect`),c=Math.max(i-r+30,50)+(b-25);s.attr(`height`,c)}r(void 0,m,u.mindmap?.padding??c.kanban.padding,u.mindmap?.useMaxWidth??c.kanban.useMaxWidth)},`draw`)},A=e(t=>{let r=``;for(let e=0;e<t.THEME_COLOR_LIMIT;e++)t[`lineColor`+e]=t[`lineColor`+e]||t[`cScaleInv`+e],n(t[`lineColor`+e])?t[`lineColor`+e]=a(t[`lineColor`+e],20):t[`lineColor`+e]=o(t[`lineColor`+e],20);let i=e((e,n)=>t.darkMode?o(e,n):a(e,n),`adjuster`);for(let e=0;e<t.THEME_COLOR_LIMIT;e++){let n=``+(17-3*e);r+=`
  ...
  .kanban-ticket-link {
    ...
    `}return r},`genSections`),j={db:O,renderer:k,parser:_,styles:e(e=>`
  .edge {
    stroke-width: 3;
  }
  ${A(e)}
