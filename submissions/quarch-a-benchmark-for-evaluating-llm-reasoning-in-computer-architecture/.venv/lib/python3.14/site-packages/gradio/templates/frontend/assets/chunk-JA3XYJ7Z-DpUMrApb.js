...
`)),a+=o+n[t+1]}),a}var _t={body:`<g><rect width="80" height="80" style="fill: #087ebf; stroke-width: 0px;"/><text transform="translate(21.16 64.67)" style="fill: #fff; font-family: ArialMT, Arial; font-size: 67.75px;"><tspan x="0" y="0">?</tspan></text></g>`,height:80,width:80},K=new Map,vt=new Map,yt=n(e=>{for(let t of e){if(!t.name)throw Error(`Invalid icon loader. Must have a "name" property with non-empty string value.`);if(r.debug(`Registering icon pack:`,t.name),`loader`in t)vt.set(t.name,t.loader);else if(`icons`in t)K.set(t.name,t.icons);else throw r.error(`Invalid icon loader:`,t),Error(`Invalid icon loader. Must have either "icons" or "loader" property.`)}},`registerIconPacks`),bt=n(async(e,t)=>{let n=m(e,!0,t!==void 0);if(!n)throw Error(`Invalid icon name: ${e}`);let i=n.prefix||t;if(!i)throw Error(`Icon name must contain a prefix: ${e}`);let a=K.get(i);if(!a){let e=vt.get(i);if(!e)throw Error(`Icon set not found: ${n.prefix}`);try{a={...await e(),prefix:i},K.set(i,a)}catch(e){throw r.error(e),Error(`Failed to load icon set: ${n.prefix}`)}}let o=ae(a,n.name);if(!o)throw Error(`Icon not found: ${e}`);return o},`getRegisteredIconData`),xt=n(async e=>{try{return await bt(e),!0}catch{return!1}},`isIconAvailable`),St=n(async(e,t,n)=>{let a;try{a=await bt(e,t?.fallbackPrefix)}catch(e){r.error(e),a=_t}let o=pe(a,t);return i(ve(_e(o.body),{...o.attributes,...n}),c())},`getIconSVG`);function Ct(e,{markdownAutoWrap:t}){let n=gt(e.replace(/<br\/>/g,`
`).replace(/\n{2,}/g,`
`));return t===!1?n.replace(/ /g,`&nbsp;`):n}n(Ct,`preprocessMarkdown`);function wt(e,t={}){let r=Ct(e,t),i=G.lexer(r),a=[[]],o=0;function s(e,t=`normal`){e.type===`text`?e.text.split(`
`).forEach((e,n)=>{n!==0&&(o++,a.push([])),e.split(` `).forEach(e=>{e=e.replace(/&#39;/g,`'`),e&&a[o].push({content:e,type:t})})}):e.type===`strong`||e.type===`em`?e.tokens.forEach(t=>{s(t,e.type)}):e.type===`html`&&a[o].push({content:e.text,type:`normal`})}return n(s,`processNode`),i.forEach(e=>{e.type===`paragraph`?e.tokens?.forEach(e=>{s(e)}):e.type===`html`?a[o].push({content:e.text,type:`normal`}):a[o].push({content:e.raw,type:`normal`})}),a}n(wt,`markdownToLines`);function Tt(e,{markdownAutoWrap:t}={}){let i=G.lexer(e);function a(e){return e.type===`text`?t===!1?e.text.replace(/\n */g,`<br/>`).replace(/ /g,`&nbsp;`):e.text.replace(/\n */g,`<br/>`):e.type===`strong`?`<strong>${e.tokens?.map(a).join(``)}</strong>`:e.type===`em`?`<em>${e.tokens?.map(a).join(``)}</em>`:e.type===`paragraph`?`<p>${e.tokens?.map(a).join(``)}</p>`:e.type===`space`?``:e.type===`html`?`${e.text}`:e.type===`escape`?e.text:(r.warn(`Unsupported markdown: ${e.type}`),e.raw)}return n(a,`output`),i.map(a).join(``)}n(Tt,`markdownToHTML`);function q(e){return Intl.Segmenter?[...new Intl.Segmenter().segment(e)].map(e=>e.segment):[...e]}n(q,`splitTextToChars`);function Et(e,t){return J(e,[],q(t.content),t.type)}n(Et,`splitWordToFitWidth`);function J(e,t,n,r){if(n.length===0)return[{content:t.join(``),type:r},{content:``,type:r}];let[i,...a]=n,o=[...t,i];return e([{content:o.join(``),type:r}])?J(e,o,a,r):(t.length===0&&i&&(t.push(i),n.shift()),[{content:t.join(``),type:r},{content:n.join(``),type:r}])}n(J,`splitWordToFitWidthRecursion`);function Dt(e,t){if(e.some(({content:e})=>e.includes(`
`)))throw Error(`splitLineToFitWidth does not support newlines in the line`);return Y(e,t)}n(Dt,`splitLineToFitWidth`);function Y(e,t,n=[],r=[]){if(e.length===0)return r.length>0&&n.push(r),n.length>0?n:[];let i=``;e[0].content===` `&&(i=` `,e.shift());let a=e.shift()??{content:` `,type:`normal`},o=[...r];if(i!==``&&o.push({content:i,type:`normal`}),o.push(a),t(o))return Y(e,t,n,o);if(r.length>0)n.push(r),e.unshift(a);else if(a.content){let[r,i]=Et(t,a);n.push([r]),i.content&&e.unshift(i)}return Y(e,t,n)}n(Y,`splitLineToFitWidthRecursion`);function X(e,t){t&&e.attr(`style`,t)}n(X,`applyStyle`);async function Ot(e,t,n,r,l=!1,u=c()){let d=e.append(`foreignObject`);d.attr(`width`,`${10*n}px`),d.attr(`height`,`${10*n}px`);let f=d.append(`xhtml:div`),p=o(t.label)?await a(t.label.replace(s.lineBreakRegex,`
`),u):i(t.label,u),ee=t.isNode?`nodeLabel`:`edgeLabel`,m=f.append(`span`);m.html(p),X(m,t.labelStyle),m.attr(`class`,`${ee} ${r}`),X(f,t.labelStyle),f.style(`display`,`table-cell`),f.style(`white-space`,`nowrap`),f.style(`line-height`,`1.5`),f.style(`max-width`,n+`px`),f.style(`text-align`,`center`),f.attr(`xmlns`,`http://www.w3.org/1999/xhtml`),l&&f.attr(`class`,`labelBkg`);let h=f.node().getBoundingClientRect();return h.width===n&&(f.style(`display`,`table`),f.style(`white-space`,`break-spaces`),f.style(`width`,n+`px`),h=f.node().getBoundingClientRect()),d.node()}n(Ot,`addHtmlSpan`);function Z(e,t,n){return e.append(`tspan`).attr(`class`,`text-outer-tspan`).attr(`x`,0).attr(`y`,t*n-.1+`em`).attr(`dy`,n+`em`)}n(Z,`createTspan`);function kt(e,t,n){let r=e.append(`text`),i=Z(r,1,t);Q(i,n);let a=i.node().getComputedTextLength();return r.remove(),a}n(kt,`computeWidthOfText`);function At(e,t,n){let r=e.append(`text`),i=Z(r,1,t);Q(i,[{content:n,type:`normal`}]);let a=i.node()?.getBoundingClientRect();return a&&r.remove(),a}n(At,`computeDimensionOfText`);function jt(e,t,r,i=!1){let a=1.1,o=t.append(`g`),s=o.insert(`rect`).attr(`class`,`background`).attr(`style`,`stroke: none`),c=o.append(`text`).attr(`y`,`-10.1`),l=0;for(let t of r){let r=n(t=>kt(o,a,t)<=e,`checkWidth`),i=r(t)?[t]:Dt(t,r);for(let e of i)Q(Z(c,l,a),e),l++}if(i){let e=c.node().getBBox();return s.attr(`x`,e.x-2).attr(`y`,e.y-2).attr(`width`,e.width+4).attr(`height`,e.height+4),o.node()}else return c.node()}n(jt,`createFormattedText`);function Q(e,t){e.text(``),t.forEach((t,n)=>{let r=e.append(`tspan`).attr(`font-style`,t.type===`em`?`italic`:`normal`).attr(`class`,`text-inner-tspan`).attr(`font-weight`,t.type===`strong`?`bold`:`normal`);n===0?r.text(t.content):r.text(` `+t.content)})}n(Q,`updateTextContentAndStyles`);async function $(e,t={}){let n=[];e.replace(/(fa[bklrs]?):fa-([\w-]+)/g,(e,r,a)=>(n.push((async()=>{let n=`${r}:${a}`;return await xt(n)?await St(n,void 0,{class:`label-icon`}):`<i class='${i(e,t).replace(`:`,` `)}'></i>`})()),e));let r=await Promise.all(n);return e.replace(/(fa[bklrs]?):fa-([\w-]+)/g,()=>r.shift()??``)}n($,`replaceIconSubstring`);var Mt=n(async(n,i=``,{style:a=``,isTitle:s=!1,classes:c=``,useHtmlLabels:l=!0,isNode:u=!0,width:d=200,addSvgBackground:f=!1}={},p)=>{if(r.debug(`XYZ createText`,i,a,s,c,l,u,`addSvgBackground: `,f),l){let e=await $(t(Tt(i,p)),p),r=i.replace(/\\\\/g,`\\`);return await Ot(n,{isNode:u,label:o(i)?r:e,labelStyle:a.replace(`fill:`,`color:`)},d,c,f,p)}else{let t=jt(d,n,wt(i.replace(/<br\s*\/?>/g,`<br/>`).replace(`<br>`,`<br/>`),p),i?f:!1);if(u){/stroke:/.exec(a)&&(a=a.replace(`stroke:`,`lineColor:`));let n=a.replace(/stroke:[^;]+;?/g,``).replace(/stroke-width:[^;]+;?/g,``).replace(/fill:[^;]+;?/g,``).replace(/color:/g,`fill:`);e(t).attr(`style`,n)}else{let n=a.replace(/stroke:[^;]+;?/g,``).replace(/stroke-width:[^;]+;?/g,``).replace(/fill:[^;]+;?/g,``).replace(/background:/g,`fill:`);e(t).select(`rect`).attr(`style`,n.replace(/background:/g,`fill:`));let r=a.replace(/stroke:[^;]+;?/g,``).replace(/stroke-width:[^;]+;?/g,``).replace(/fill:[^;]+;?/g,``).replace(/color:/g,`fill:`);e(t).select(`text`).attr(`style`,r)}return t}},`createText`);export{$ as a,yt as i,Mt as n,_t as o,St as r,gt as s,At as t};
