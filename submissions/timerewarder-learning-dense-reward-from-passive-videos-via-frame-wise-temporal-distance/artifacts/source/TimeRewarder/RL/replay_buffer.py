    ...
    def add(self, time_step):
        for spec in self._data_specs:
            ...
            if np.isscalar(value):
                ...
                if value.shape == ():
                    value = np.full(spec.shape, value.item(), spec.dtype)
                elif spec.shape == (1,) and value.ndim == 1 and value.size == 1:
                    value = value.astype(spec.dtype)
                    value = value.reshape(spec.shape)
                elif spec.shape == value.shape:
                    value = value.astype(spec.dtype)
                else:
                    raise ValueError(
                        f"ReplayBufferStorage.add: spec {spec.name} expected shape {spec.shape} dtype {spec.dtype}, "
                        f"but got value shape {value.shape} dtype {value.dtype}"
                    )
