...
def report_page(bundle: dict) -> str:
    controls = bundle["audits"]["geometry_mechanism"]
    convergence = bundle["audits"]["alternating_convergence"]
    prefix = bundle["audits"]["prefix_vs_bruteforce"]
    relaxation = bundle["audits"]["theorem_relaxation"]
    crosscheck = bundle["audits"]["official_crosscheck"]
    return f"""# Consolidated judge-visible report

Paper `{bundle["paper_id"]}` | Attempt `{bundle["attempt_id"]}` |
Pinned upstream `{bundle["upstream_revision"]}` |
Paid API cost USD {bundle["estimated_api_cost_usd"]:.2f}

This page duplicates the claim evidence from the tabbed logbook in one
plain Markdown file so automated and manual review paths can inspect the
actual numerical outputs without traversing the Gradio tabs.

## Claim-by-claim verdicts

| Claim | Status | Evidence actually executed |
| --- | --- | --- |
| Claim 1: Wasserstein objective and embedding geometry | {bundle["claim_results"]["claim_1"]["status"]} | f-step surrogate max error {controls["potential_max_error"]:.1e}; uniform-metric prefix reduction {controls["uniform_metric_prefix_matches"]}/{controls["trials"]}; alternating convergence {convergence["converged"]}/{convergence["trials"]} |
| Claim 2: exact subset update inside the alternating decoder | {bundle["claim_results"]["claim_2"]["status"]} | prefix scan matched brute force {prefix["optimal_value_matches"]}/{prefix["trials"]}; {prefix["subsets_enumerated_per_trial"]} subsets enumerated per trial; official-code kept-set match {crosscheck["identical_kept_sets"]}/{crosscheck["trials"]} |
| Claim 3: GSM8K table | {bundle["claim_results"]["claim_3"]["status"]} | no language-model benchmark was run; synthetic distribution-shaping metrics only |
| Claim 4: GPQA table | {bundle["claim_results"]["claim_4"]["status"]} | not run; official `run_gpqa.sh` entry point identified and pinned |
| Claim 5: AlpacaEval / MT-Bench win rates | {bundle["claim_results"]["claim_5"]["status"]} | not run; no generations or judge calls were made |

## Numerical mechanism audits

- Claim 1 f-step surrogate: max absolute error
  **{controls["potential_max_error"]:.1e}** against a naive
  per-token nearest-set computation.
- Claim 1 convergence: **{convergence["converged"]}/{convergence["trials"]}**
  random 500-token instances converged within the 9-iteration budget
  (mean {convergence["mean_iterations"]:.2f}, max
  {convergence["max_iterations"]} iterations).
- Claim 1 uniform-metric control: **{controls["uniform_metric_prefix_matches"]}/{controls["trials"]}**
  trials reduced exactly to a top-probability prefix when embeddings
  were identical.
- Claim 1 geometry sensitivity control: default official
  hyperparameters yielded probability-prefix matches in
  **{controls["probability_prefix_matches"]}/{controls["trials"]}**
  synthetic clustered trials and mean shuffled-embedding Jaccard
  **{controls["mean_shuffle_jaccard"]:.3f}**.
- Claim 2 exact S-step audit: the prefix scan matched brute-force
  enumeration in **{prefix["optimal_value_matches"]}/{prefix["trials"]}**
  trials with max objective gap **{prefix["max_objective_gap"]:.2e}**.
- Claim 2 brute-force control enumerated
  **{prefix["subsets_enumerated_per_trial"]}** nonempty subsets per
  trial, versus a one-sort prefix scan.
- Claim 2 theorem-hypothesis control: when `beta < lam`, pure-prefix
  selection became suboptimal in
  **{relaxation["prefix_suboptimal_instances"]}/{relaxation["trials"]}**
  trials (worst gap {relaxation["worst_objective_gap"]:.3f}), showing
  the paper's `beta - lam >= 0` condition is load-bearing.
- Claim 2 official implementation cross-check: reimplementation kept
  identical token sets to vendored `logit_processor_w1.py` in
  **{crosscheck["identical_kept_sets"]}/{crosscheck["trials"]}** trials
  with mean kept size {crosscheck["mean_kept_size"]:.1f}.

## Synthetic distribution-shaping context

These values are diagnostic only and are not benchmark accuracy:

{metric_rows(bundle)}

## Explicit non-replications

Claim 3: unreplicated. {bundle["claim_results"]["claim_3"]["evidence"]}

Claim 4: unreplicated. {bundle["claim_results"]["claim_4"]["evidence"]}

Claim 5: unreplicated. {bundle["claim_results"]["claim_5"]["evidence"]}

## Provenance

The official repository is pinned at
`{bundle["upstream"]["repository"]}@{bundle["upstream"]["revision"]}`.
The machine-readable source of this report is `evidence/bundle.json`,
regenerated by:
