...
class _LayerNormMLP(torch.autograd.Function):
    ...
    def forward(
        ...
        def norm_fun(_inp,out_):
            out_, mu, rsigma = _apply_normalization(
                _inp,
                out_,
                ln_weight,
                ln_bias,
        ...
        if fp8:
            ...
            if bias_gelu_nvfusion:
                ...
                def act_fun(_inp):
                    return bias_gelu_fused(_inp, fc1_bias)
                ...
                if activation == "gelu":
                    ...
                    def act_fun(_inp):
                        return activation_func(
                                _inp, None, tex.FP8FwdTensors.GEMM2_INPUT, TE_DType[fc1_out.dtype]
                            )
                    gelu_out = activation_func(
        ...
        if is_grad_enabled:
            ...
            q_input, q_bit, q_scale, q_min = op_quantize(inputmat.half(), 8, 1)
            q_fc1_out, q_fc1_bit, q_fc1_scale, q_fc1_min = op_quantize(fc1_out.half(), 8, 1)
            ctx.save_for_backward(
                q_input,
                ln_weight,
                mu,
                rsigma,
                None,# ln_out if fc1_weight.requires_grad else None,
                q_fc1_out,
                None,#gelu_out if fc2_weight.requires_grad else None,
                fc1_weight,
                fc1_weight_fp8,
            ...
            ctx.q_fc1_bit, ctx.q_fc1_scale, ctx.q_fc1_min = q_fc1_bit, q_fc1_scale, q_fc1_min
            ctx.norm_fun = norm_fun
            ctx.act_fun = act_fun
            ctx.input_shape = inputmat.shape
            ctx.fc1_shape = fc1_out.shape
    ...
    def backward(
        ...
        with torch.cuda.nvtx.range("_LayerNormMLP_backward"):
            ...
            q_inputs = [q_input, ctx.q_bit, ctx.q_scale, ctx.q_min]
            norm_fun = ctx.norm_fun
            inputmat = op_dequantize(q_inputs, ctx.input_shape).to(torch.bfloat16)
            ln_out = torch.empty_like(inputmat, memory_format=torch.contiguous_format)
            ln_out = norm_fun(inputmat,ln_out)

            q_fc1_outs = [q_fc1_out, ctx.q_fc1_bit, ctx.q_fc1_scale, ctx.q_fc1_min]
            act_fun = ctx.act_fun
            fc1_out = op_dequantize(q_fc1_outs, ctx.fc1_shape).to(torch.bfloat16)
            gelu_out = act_fun(fc1_out)
