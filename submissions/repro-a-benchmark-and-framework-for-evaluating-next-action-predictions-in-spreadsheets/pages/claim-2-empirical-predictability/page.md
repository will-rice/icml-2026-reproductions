...
---
<!-- trackio-cell
{"type": "code", "id": "cell_6a14daab69c8", "created_at": "2026-07-21T15:59:45+00:00", "title": "Aggregate all released oracle outputs", "command": ["uv", "run", "pytest", "tests/test_predictability_evidence.py", "-q"], "exit_code": 0, "duration_s": 0.66}
-->
````bash
$ uv run pytest tests/test_predictability_evidence.py -q
````

exit 0 · 0.7s


````python title=test_predictability_evidence.py
import json
...
---
<!-- trackio-cell
{"type": "markdown", "id": "cell_3f9f7724dcd5", "created_at": "2026-07-21T16:05:21+00:00", "title": "Outcome and evidence scope"}
-->
**REPRODUCED FROM RELEASED OUTPUTS.** Aggregating all 52 released `predictable_state.json` files yields **126,940 / 186,574 = 68.04%** weighted predictable-property coverage. Per-trajectory coverage has mean **65.99%**, median **66.34%**, and **44/52** trajectories above 50%.

This independently recomputes the aggregate from released oracle outputs. The original paid frontier-model oracle calls were **not rerun**, so the result does not establish independent regeneration of those outputs. Counting definitions, paths, and revision are in `claim_2_predictability.json` in the attached bundle.

**Independent CPU Job provenance.** [Job `wrice/6a61607c13e6ef894d54c828`](https://huggingface.co/jobs/wrice/6a61607c13e6ef894d54c828) reran this released-oracle-output recomputation without paid-model credentials; the original paid oracle calls remain not rerun. The [generated JSON](https://huggingface.co/buckets/wrice/nape-cpu-job-evidence-7220279/resolve/nape-7220279222f1-claims-1-3/claim_2_predictability.json?download=true) has SHA-256 `8cbdf811fc2bc3a8639475809c5148859ede0078a4439f5a3c6997b98c854904` and matched a freshly regenerated local bundle byte-for-byte.
